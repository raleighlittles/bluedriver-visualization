...
def rename_log_file_using_date(log_full_path: str, new_log_filename_suffix="__BlueDriverLog.csv"):
    ...
    with open(log_full_path, 'r') as log_file_handle:
        ...
        log_file_timestamp = datetime.datetime.strptime(
            file_timestamp, bluedriver_timestamp_format).strftime(desired_timestamp_format)
    ...
    print(
        f"[DEBUG] Renamed {os.path.basename(log_full_path)} to {new_log_file_full_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input-folder",
                        help="Input directory: where Bluedriver data files are stored", type=str, required=True)
    parser.add_argument("-o", "--output-folder",
                        help="Output directory: where output graphs will be stored", type=str, required=True)
    parser.add_argument("-r", "--rename", action="store_true",
                        help="Whether or not you'd like the files to be renamed", required=True)

    args = parser.parse_args()

    generate_bluedriver_plots_from_data(
